...
class TestAccountProfilePage:
    @allure.title('Переход переход в Личный кабинет по клику на кнопку Личный кабинет')
    ...
    @allure.title('Переход в раздел История заказов по клику на гиперссылку История заказов')
    ...
    @allure.title('Выход из аккаунта по клику на кнопку Выход в Личном кабинете')
    def test_go_to_login_page_from_account_profile_page_by_button_click(self, logged_in_main_page_driver):
        main_page = MainPage(logged_in_main_page_driver)
        main_page.click_personal_account_href()
        account_profile_page = AccountProfilePage(logged_in_main_page_driver)
        account_profile_page.click_exit_button()
        current_url = logged_in_main_page_driver.current_url
